...
#. 共识出块节点的选择上，CITA使用了轮询的方法，而不是通过pow算力选择。例如节点数N，高度为H的出块节点为 H mod N。
#. CITA优化了commit阶段的广播，使用Chain模块的状态消息作为出块成功的标志。
#. Tendermint为了在投票上更好的与其它节点在时间上投票，在2/3票数后，会进行必要的超时等待慢速节点。而CITA则可以减少甚至舍弃超时等待的时间。
